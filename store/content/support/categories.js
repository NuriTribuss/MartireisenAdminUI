...
const actions = {
  ...
  async delete({ commit, dispatch }, { id }) {
    ...
    if (id.length === 0) {
      ...
      for (let i = 0; i < id.length; i++) {
        const { data } =   await this.$axios.delete(endpoints['content_support_category']+'/'+id[i]);
        if (data.status) {
  ...
  async fetchChild({ commit }, { id }) {
    commit('setLoading', true)
    const res = await  this.$axios.get(endpoints['content_support_category']+'/'+id+'/children');
